...
/*
function underFive(bikes) {

    let resultPlace = []

    for (let i = 0; i < bikes.length; i++) {
        let countBike = (bikes[i]['parkingBikeTotCnt'] <= 5)

        if (countBike) {
            resultPlace.push(bikes[i]['stationName'])
        }
    }
    return resultPlace
}

exports.underFive = underFive

function printPlace (bikes) {
  let resultPlace = underFive (bikes)
...
function bikeCount(bikeCount) {
    for(let i = 0 ; i < bikes.length ; i++ ){
        if(bikes[i]['parkingBikeTotCnt'] <= bikeCount) {
            let station = bikes[i]['stationName']
            console.log(station)
        }
    }
